...
def load_run(run_dir: Path) -> pd.DataFrame:
    """Carga el resumen de evaluación de una corrida/semilla."""
    csv_path = run_dir / "evaluation_summary.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"No existe {csv_path}. Primero corre evaluate.py para esta semilla.")
...
def plot_all_points(df: pd.DataFrame, out_path: Path) -> None:
    """Grafica todos los puntos por semilla y el frente no dominado global."""
    ...
    plt.title("Puntos evaluados con múltiples semillas")
    plt.legend()
    plt.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Agrega resultados de varias semillas.")
    parser.add_argument("--runs", nargs="+", required=True, help="Directorios de cada semilla, por ejemplo results/seed_0 results/seed_1.")
    parser.add_argument("--out-dir", type=str, required=True, help="Directorio donde se guardará el resumen agregado.")
    ...
    metric_cols = [c for c in all_df.columns if c.startswith("mean_obj_")]
    metric_cols += ["mean_scalarized_return", "mean_episode_length"]

    agg = all_df.groupby(["weight_name", "weight"], as_index=False)[metric_cols].agg(["mean", "std"])
    agg.columns = ["_".join(col).strip("_") for col in agg.columns.to_flat_index()]
    agg = agg.reset_index()
    agg.to_csv(out_dir / "aggregated_by_weight.csv", index=False)
    ...
    if len(obj_cols) == 2:
        ...
        plot_all_points(all_df, out_dir / "multi_seed_pareto_front.png")

    save_json(out_dir / "aggregate_metrics.json", metrics)

    pd.set_option("display.float_format", lambda x: f"{x:.3f}")
    print("Resumen agregado por peso:")
