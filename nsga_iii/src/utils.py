...
def make_env(env_id: str, seed: int | None = None, max_episode_steps: int | None = None):
...
def infer_reward_dim(env) -> int:
    if hasattr(env.unwrapped, "reward_space"):
        return int(np.prod(env.unwrapped.reward_space.shape))
    obs, _ = env.reset()
...
def smallest_multiple_of_four_at_least(n: int) -> int:
