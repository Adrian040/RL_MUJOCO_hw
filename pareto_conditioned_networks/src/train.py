...
def select_training_target(episodes: List[Dict], reward_dim: int, exploration_scale: float, rng: np.random.Generator) -> Tuple[np.ndarray, int]:
    ...
    if len(nd_idx) == 0:
        ...
        idx = int(rng.choice(nd_idx))

    base_return = np.asarray(episodes[idx]["return_vec"], dtype=np.float32).copy()
    horizon = int(episodes[idx]["length"])
    front = returns_array([episodes[i] for i in nd_idx]) if len(nd_idx) > 0 else returns_array(episodes)
    if len(front) > 1:
        sigma = np.maximum(front.std(axis=0), 1e-6)
    else:
        sigma = np.maximum(np.abs(base_return), 1.0) * 0.05
...
def train_network(
    ...
    for _ in range(updates):
        obs, cond, actions, _ = sample_batch(episodes, batch_size, rng)
        obs = normalize_obs(obs, normalizer)
        cond = normalize_condition(cond, normalizer)
...
def main() -> None:
    ...
    model = PCNPolicy(
        obs_dim=obs_dim,
        condition_dim=reward_dim + 1,
        num_actions=len(action_bank),
    ...
    episodes = prune_episodes(
        episodes,
        int(config["max_trajectories"]),
        float(config["crowding_threshold"]),
        float(config["crowding_penalty"]),
    )
    ...
    for iteration in tqdm(range(1, int(config["iterations"]) + 1), desc="Entrenando PCN"):
        ...
        loss = train_network(
            ...
            updates=int(config["updates_per_iteration"]),
        ...
        for _ in range(int(config["episodes_per_iteration"])):
            target_return, target_horizon = select_training_target(
                episodes,
                reward_dim=reward_dim,
                exploration_scale=float(config["exploration_scale"]),
                rng=rng,
            )
            episodes.append(
                collect_pcn_episode(
                    ...
                    device=device,
                    stochastic=bool(config.get("stochastic_train_policy", True)),
                )
            )
    ...
    torch.save(
        ...
        run_dir / "checkpoints" / "pcn_final.pt",
    )
    env.close()
    print("Entrenamiento terminado.")
    print(f"Checkpoint: {run_dir / 'checkpoints' / 'pcn_final.pt'}")
    print(f"Cobertura: {run_dir / 'dataset_coverage.csv'}")
